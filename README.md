# Hi 👋, I'm Kunal Sharma

### Full Stack Developer | AI Enthusiast | C++ (DSA)

I'm a Computer Science undergraduate passionate about building scalable web applications, AI-powered solutions, and interactive user experiences. I enjoy solving Data Structures & Algorithms problems and continuously learning modern technologies.

---

## 🚀 About Me

* 💻 Full Stack Developer
* 🤖 Exploring AI & Computer Vision
* 🧩 Solving DSA in C++
* 🌱 Currently learning System Design & Advanced Backend
* 🎯 Goal: Become a Software Development Engineer

---

## 🛠 Tech Stack

### Full Stack

* NextJs
* MERN Stack

### Languages

* C++
* JavaScript (ES6+)
* Python
* C

### Frontend

* React.js
* HTML5
* CSS3
* Tailwind CSS
* Bootstrap
* GSAP
* shadcn/ui

### Backend

* Node.js
* Express.js
* Django
* EJS

### Database

* MongoDB

### Cloud , library & Tools

* Git
* GitHub
* Vercel
* Render
* Netlify
* Cloudinary
* Resend
* Zod
* Kubernetes

---

# 💼 Experience

### Software Developer Intern — PMKVY

* Developed interactive web applications using modern technologies.
* Followed software engineering standards and clean coding practices.

### Web Developer Intern — Business 365 Advisors

* Built responsive applications using Next.js and GSAP.
* Collaborated in a 5-member Agile team.
* Optimized performance and deployed projects on Vercel.
* Designed pixel-perfect responsive interfaces.

### SIG Web Development Head — CSI (IPEC)

* Conducted web development workshops.
* Mentored students in React, Git, GitHub, and modern JavaScript.
* Guided live project development.

### Technical Head

Trinity Cyber Forum • CSI • Arcadia Society

* Leading technical teams.
* Organizing coding events.
* Managing web projects and technical workshops.

---

# 🚀 Featured Projects

## 🎓 OptiRoll

AI-powered Attendance Management System

### Features

* Face Recognition using FaceNet
* Live Attendance Tracking
* Admin, Teacher & Student Dashboards
* 2FA Authentication
* Email Notifications
* Attendance Analytics

**Tech Stack**

React • Node.js • Express • MongoDB • Python • FaceNet • Cloudinary • Resend • Kubernetes • Vercel • Render

🔗 [**Live Demo**](https://opti-roll.vercel.app/)

📂 [**Source Code**](https://github.com/LISTEDMILE/OptiRoll)

---

## 💼 HireSphere

A modern recruitment platform featuring

* Job Posting
* Candidate Applications
* Talent Pool
* Recruiter Dashboard
* Hiring Workflow

**Tech**

MERN • GSAP • Cloudinary

🔗 [**Live Demo**](https://hire-sphere-sy.vercel.app/)

📂 [**Source Code**](https://github.com/LISTEDMILE/HireSphere)

---

## ❤️ UsStory

A storytelling platform where users can

* Create story pages
* Upload media
* Add music
* Set public/private visibility
* Tag relationships

Built with React, Express, MongoDB and GSAP.

🔗 [**Live Demo**](https://us-story.vercel.app/)

📂 [**Source Code**](https://github.com/LISTEDMILE/UsStory)

---

## 🌐 Business365 Advisors

Animated corporate website built using

* Next.js
* GSAP
* Tailwind CSS

🔗 [**Live Demo**](https://www.b365advisors.com/)

---

## 🧮 Base Number Converter & Calculator

Supports

* Binary
* Decimal
* Octal
* Hexadecimal

Includes conversion and arithmetic operations.

🔗 [**Live Demo**](https://basechange.netlify.app/)

📂 [**Source Code**](https://github.com/LISTEDMILE/BaseChange)

---

# 🏆 Achievements

* Full Stack Web Development with AI Tools by Edunet Foundation
* IoT for Cyber Physical Systems – IHUB Divya Sampark, IIT Roorkee
* AWS Academy Cloud Foundations
* Oracle Database Programming with SQL
* MongoDB Certification
* Cisco Introduction to Cybersecurity
* EY & Microsoft AI Skills Passport
* NASSCOM Generative AI Literacy
* Software Programmer (PMKVY)
* Sig WebDev - CSI Society of India.
---
* 3rd place, Silver Jubilee Poster Presentation (2022 & 2023)

---

# 📈 2026 Goals

* Solve 500+ LeetCode problems
* Contribute to Open Source
* Learn System Design
* Build scalable AI-powered applications
* Crack an SDE role

---

# 📚 Coding Profiles

* LeetCode : https://leetcode.com/u/Kunal_LISTEDMILE/
* Coding Ninjas : https://www.naukri.com/code360/profile/KunalListedmile

Currently solving DSA in C++ consistently.

---

## 📫 Connect With Me

- 🌐 **Portfolio:** [portfoliokunalsharma.vercel.app](https://portfoliokunalsharma.vercel.app/)
- 💼 **LinkedIn:** [Kunal Sharma](https://www.linkedin.com/in/kunal-sharma-cse/)
- 🐙 **GitHub:** [LISTEDMILE](https://github.com/LISTEDMILE)
- 📧 **Email:** <listedmile@gmail.com>
- 
⭐ If you like my work, consider starring my repositories!
